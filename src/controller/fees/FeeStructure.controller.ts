...
export const createFeeStructure = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!feeType) throw new ErrorResponse("Fee Type not found", status.Not_Found);
    if (!academicYear) throw new ErrorResponse("Academic Year not found", status.Not_Found);
    if (!schoolClass) throw new ErrorResponse("Class not found", status.Not_Found);
    if (!school) throw new ErrorResponse("School not found", status.Not_Found);
...
export const getAllFeeStructures = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (feeTypeId) where.feeTypeId = String(feeTypeId);
...
export const updateFeeStructure = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!existing) throw new ErrorResponse("Fee Structure not found", status.Not_Found);
